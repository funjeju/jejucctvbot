          ...
          {currentMedia && (
            <>
              <div className="relative w-full flex-shrink-0 flex items-center justify-center">
                ...
                {/* EXIF 오버레이 (세로 이미지 또는 PC일 때만) */}
                {currentMedia.exif && !isLandscape && (
                  <div className="absolute bottom-4 left-4 right-4 top-auto bg-black bg-opacity-50 text-white rounded-lg p-3 space-y-2 text-xs max-h-[40vh] overflow-y-auto lg:block">
                  {/* 촬영 날짜/시간 + 업로드 시간 (한 줄) */}
