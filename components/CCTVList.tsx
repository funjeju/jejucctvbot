...
const CCTVList: React.FC<CCTVListProps> = ({ cctvs, selectedCCTV, onSelect, language }) => {
  ...
  return (
    <div className="bg-white shadow-xl rounded-xl p-3 sm:p-5 border-2 border-blue-300">
      {/* 지역 선택 탭 */}
      ...
      {selectedCCTV && selectedCCTV.description && (
        <div className="mt-4 p-4 bg-blue-50 rounded-xl border-2 border-blue-300">
          <p className="text-sm text-blue-700 font-medium">{selectedCCTV.description}</p>
        </div>
      )}
    </div>
  );
